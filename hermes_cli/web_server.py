...
def _ws_host_origin_reason(ws: "WebSocket") -> Optional[str]:
    ...
    host_header = ws.headers.get("host", "")
    if not any(_is_accepted_host(host_header, host) for host in allowed_hosts):
        return f"host_mismatch host={host_header or '?'} bound={','.join(allowed_hosts)}"
    ...
    if not parsed.netloc:
        return f"origin_mismatch origin={origin} bound={','.join(allowed_hosts)}"

    if not any(_is_accepted_host(parsed.netloc, host) for host in allowed_hosts):
        return f"origin_mismatch origin={origin} bound={','.join(allowed_hosts)}"
    return None
